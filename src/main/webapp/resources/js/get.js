 ...
 $(function(){
	...
	function showReplyList(list){
		...
		for(var reply of list){
			...
			<li class="media" id="reply${reply.rno}" data-rno="${reply.rno}">
				<div class="media-body">
					...
					<p>${reply.reply}</p>
					<small>${new Date(reply.replyDate).toISOString().split("T")[0]}</small><hr>
				</div>
			</li>`;
		...
		mediaBody.append("<h5 class='my-4'>" + reply.replyer + "</h5>")
			   ...
			   .append("<small>" + new Date(reply.replyDate).toISOString().split("T")[0] + "</small><hr>")
	...
	$("#reply-insert-btn1").click(function(){
		...
		$.ajax({
			...
			success: function(){
				...
				getReplyList();
				//alert("새 댓글 입력하였습니다.");
				$("#alert1").text("새 댓글 작성하였습니다.").addClass("show");
	...
	$("#reply-modify-btn1").click(function(){
		...
		$.ajax({
			...
			success: function(){
				...
				$('#reply-insert-modal').modal('hide');
				getReplyList();
				$("#alert1").text("댓글 수정 하였습니다.").addClass("show");
			},
			error: function(){
				console.log("수정 실패")
			}
		...
		var data = {
			...
			$.ajax({
				...
				success: function(){
					...
					$('#reply-insert-modal').modal('hide');
					getReplyList();
					$("#alert1").text("댓글 삭제 하였습니다.").addClass("show");
				},
				error: function(){
					console.log("수정 실패")
				}
